...
class EmailScorer(models.AbstractModel):
    _name = 'email.scorer'
    _description = 'Email Field Scorer'

    def score_field(self, field_value, api_key):
        """Score email field using AI"""
        if not field_value:
            return {'score': 0.0, 'reason': 'No email provided'}

        prompt = f"""
        Analyze this email address for business credibility and score out of 100:
        Email: {field_value}
